...
def get_id(con, cursor, table, field, value, force_new=False):
    ...
    if len(row) == 0 or force_new:
        ...
        cursor.execute(
            "INSERT INTO {} (rowid, {}) VALUES (?,?)".format(table, field), (id_, value)
        )
        con.commit()
...
def register_manifest(
    con, cursor, data, pathParts, manifest, manifestFilename, manifestHash: bytes
):
    # IDS
    ...
    for part in pathParts[1:]:
        ...
        cursor.execute(
            "UPDATE pathparts SET parent={} WHERE rowid={};".format(
    ...
    # NORM_NAMES
    norm_name = get_id(
        con, cursor, "norm_names", "norm_name", normalize(data["PackageName"])
    )
    cursor.execute(
    ...
    norm_publisher = get_id(
        con, cursor, "norm_publishers", "norm_publisher", normalize(data["Publisher"])
    )
    cursor.execute(
    ...
    if "Tags" in data:
        for _tag in data["Tags"]:
            ...
            cursor.execute(
                "INSERT INTO tags_map (manifest, tag) VALUES (?,?)", (manifest, tag)
            )
            con.commit()
    ...
    if "Installers" in data:
        if "PackageFamilyName" in data["Installers"][0]:
            pfn = get_id(
                con, cursor, "pfns", "pfn", data["Installers"][0]["PackageFamilyName"]
            )
            cursor.execute(
                "INSERT INTO pfns_map (manifest, pfn) VALUES (?,?)", (manifest, pfn)
            )
            con.commit()
...
def create_catalog(con):
    ...
    cursor.execute(
        "UPDATE metadata SET value=? WHERE name=?;", (int(time.time()), "lastwritetime")
    )
    con.commit()
